-- WhiteLabelEcommerce - Schema com IDs Numéricos
-- Versão otimizada para administração e performance
...
-- Mantém UUID apenas para integração com Supabase Auth
...
-- Tabela de configurações WhiteLabel (para multi-tenant)
...
-- Adicionar tenant_id aos usuários para multi-tenant
ALTER TABLE public.users ADD COLUMN tenant_id INTEGER REFERENCES public.tenant_configs(id) ON DELETE SET NULL;
